from dataclasses import dataclass
from typing import Callable, List, Tuple, Dict, Any

import numpy as np
import pandas as pd
import streamlit as st


# =========================
# FIXED REQUIREMENTS (Q1b)
# =========================
POP_SIZE = 300
GENE_LENGTH = 80
GENERATIONS = 50
SEED = 42

# (You may keep these adjustable if lecturer allows, but fixed is safest)
CROSSOVER_RATE = 0.9
MUTATION_RATE = 0.01
TOURNAMENT_K = 3
ELITISM = 2


# -------------------- Problem Definition --------------------
@dataclass
class GAProblem:
    name: str
    chromosome_type: str  # "bit"
    dim: int
    bounds: Tuple[float, float] | None
    fitness_fn: Callable[[np.ndarray], float]


def make_onemax_peak(dim: int, target_ones: int = 40, max_fitness: int = 80) -> GAProblem:
    """
    Fitness peaks at target_ones with maximum = max_fitness.
    Example: ones=40 -> 80 (max), ones=0 or 80 -> 0
    """
    def fitness(x: np.ndarray) -> float:
        ones = int(np.sum(x))
        return float(max_fitness - 2 * abs(ones - target_ones))

    return GAProblem(
        name=f"OneMax Peak ({dim} bits, peak @ {target_ones} ones)",
        chromosome_type="bit",
        dim=dim,
        bounds=None,
        fitness_fn=fitness,
    )


# -------------------- GA Operators --------------------
def init_population(problem: GAProblem, pop_size: int, rng: np.random.Generator) -> np.ndarray:
    return rng.integers(0, 2, size=(pop_size, problem.dim), dtype=np.int8)


def evaluate(pop: np.ndarray, problem: GAProblem) -> np.ndarray:
    return np.array([problem.fitness_fn(ind) for ind in pop], dtype=float)


def tournament_selection(fitness: np.ndarray, k: int, rng: np.random.Generator) -> int:
    idxs = rng.integers(0, fitness.size, size=k)
    best = idxs[np.argmax(fitness[idxs])]
    return int(best)


def one_point_crossover(a: np.ndarray, b: np.ndarray, rng: np.random.Generator) -> Tuple[np.ndarray, np.ndarray]:
    if a.size <= 1:
        return a.copy(), b.copy()
    point = int(rng.integers(1, a.size))
    c1 = np.concatenate([a[:point], b[point:]])
    c2 = np.concatenate([b[:point], a[point:]])
    return c1, c2


def bit_mutation(x: np.ndarray, mut_rate: float, rng: np.random.Generator) -> np.ndarray:
    mask = rng.random(x.shape) < mut_rate
    y = x.copy()
    y[mask] = 1 - y[mask]
    return y


def run_ga(
    problem: GAProblem,
    pop_size: int,
    generations: int,
    crossover_rate: float,
    mutation_rate: float,
    tournament_k: int,
    elitism: int,
    seed: int,
) -> Dict[str, Any]:
    rng = np.random.default_rng(seed)
    pop = init_population(problem, pop_size, rng)
    fit = evaluate(pop, problem)

    history_best: List[float] = []
    history_avg: List[float] = []
    history_worst: List[float] = []

    for _ in range(generations):
        # Logging
        history_best.append(float(np.max(fit)))
        history_avg.append(float(np.mean(fit)))
        history_worst.append(float(np.min(fit)))

        # Elitism
        E = max(0, min(elitism, pop_size))
        elite_idx = np.argpartition(fit, -E)[-E:] if E > 0 else np.array([], dtype=int)
        elites = pop[elite_idx].copy() if E > 0 else np.empty((0, pop.shape[1]), dtype=np.int8)

        # Next generation
        next_pop: List[np.ndarray] = []
        while len(next_pop) < pop_size - E:
            i1 = tournament_selection(fit, tournament_k, rng)
            i2 = tournament_selection(fit, tournament_k, rng)
            p1, p2 = pop[i1], pop[i2]

            # Crossover
            if rng.random() < crossover_rate:
                c1, c2 = one_point_crossover(p1, p2, rng)
            else:
                c1, c2 = p1.copy(), p2.copy()

            # Mutation
            c1 = bit_mutation(c1, mutation_rate, rng)
            c2 = bit_mutation(c2, mutation_rate, rng)

            next_pop.append(c1)
            if len(next_pop) < pop_size - E:
                next_pop.append(c2)

        pop = np.vstack([np.array(next_pop, dtype=np.int8), elites]) if E > 0 else np.array(next_pop, dtype=np.int8)
        fit = evaluate(pop, problem)

    best_idx = int(np.argmax(fit))
    best = pop[best_idx].copy()
    best_fit = float(fit[best_idx])

    history_df = pd.DataFrame({"Best": history_best, "Average": history_avg, "Worst": history_worst})

    return {
        "best": best,
        "best_fitness": best_fit,
        "history": history_df,
        "final_population": pop,
        "final_fitness": fit,
    }


# =======================
# Streamlit UI (Q1b)
# =======================
st.set_page_config(page_title="GA OneMax Peak (Lab Test Q1)", page_icon="🧬", layout="wide")
st.title("Genetic Algorithm – OneMax Peak (Lab Test Q1)")
st.caption("Fixed settings: Population=300, Chromosome length=80, Generations=50. Fitness peaks at 40 ones (max=80).")

# Show fixed parameters clearly
st.info(
    f"**Fixed Parameters**\n\n"
    f"- Population Size: **{POP_SIZE}**\n"
    f"- Chromosome Length: **{GENE_LENGTH}**\n"
    f"- Generations: **{GENERATIONS}**\n"
    f"- Crossover Rate: **{CROSSOVER_RATE}**\n"
    f"- Mutation Rate: **{MUTATION_RATE}**\n"
    f"- Tournament Size: **{TOURNAMENT_K}**\n"
    f"- Elitism: **{ELITISM}**\n"
    f"- Seed: **{SEED}**"
)

problem = make_onemax_peak(GENE_LENGTH, target_ones=40, max_fitness=80)

left, right = st.columns([1, 1])

# Session state to store last run
if "result" not in st.session_state:
    st.session_state["result"] = None

with left:
    if st.button("Run GA", type="primary"):
        result = run_ga(
            problem=problem,
            pop_size=POP_SIZE,
            generations=GENERATIONS,
            crossover_rate=CROSSOVER_RATE,
            mutation_rate=MUTATION_RATE,
            tournament_k=TOURNAMENT_K,
            elitism=ELITISM,
            seed=SEED,
        )
        st.session_state["result"] = result

    if st.session_state["result"] is not None:
        result = st.session_state["result"]

        st.subheader("📈 Fitness Over Generations")
        st.line_chart(result["history"])

        st.subheader("🏆 Best Solution")
        st.write(f"Best fitness: **{result['best_fitness']:.2f}** / 80")

        best_bits = result["best"].astype(int)
        bitstring = "".join(map(str, best_bits.tolist()))
        ones_count = int(np.sum(best_bits))

        st.code(bitstring, language="text")
        st.write(f"Number of ones in best chromosome: **{ones_count} / {GENE_LENGTH}**")

        st.success("Expected behaviour: GA converges toward ~40 ones so best fitness approaches 80.")

with right:
    st.subheader("Population Snapshot (final)")
    st.caption("Shows first 20 individuals with their fitness after the last generation.")

    if st.session_state["result"] is None:
        st.info("Run GA first to view the final population.")
    else:
        result = st.session_state["result"]
        pop = result["final_population"]
        fit = result["final_fitness"]

        nshow = min(20, pop.shape[0])
        df = pd.DataFrame(pop[:nshow])
        df["fitness"] = fit[:nshow]
        st.dataframe(df, use_container_width=True)
